...
\author{Jason Thomas \& Sam Clark} % Others? suggest: "in collaboartion with the Bloomberg Data for Health Inittaive". the names you have listed dont match the names listed in the Shiny app browesr window (alos have Richard and Tyler)
\maketitle
...
\begin{itemize}
\item \R software environment -- \url{https://cran.r-project.org/} %dash dont show
...
\item Install the CrossVA package (included in the \R Shiny zip file) by clicking, in the \R menu bar, \BF{
    Packages\,>\,Install\,package(s)\,from\,local\,files\ldots} then select the CrossVA\_0.9.3.zip file (you may need to %is there no need to unzip?
...
(\url{https://github.com/richardli/InSilicoVA/blob/master/Documents/Insilico-manual.pdf}). As a common work-around to problems, ensure to install both the
64 and 32 bit version of java. % assuming that is correct?
...
\item {\bf Select Algorithm}
  \begin{enumerate}
  \item {\bf InSilico} -- this algorithm computes COD by carrieing out a specific number of iterration %improve language here; include link as for InterVA
  the more iterrations the longer the time for the computation of the COD and the more precise the computation. %correct?
  The number of iterations in the simulations can be specified in the Shiny app; the default is 5,000, but
    ...
    a Java Progress Bar indicating the percentage of iterations that have been completed.  
    \begin{center}
    ...
    additional iterations to the simulation.)
  ...
  \item \vapkg includes additional algorithms, namely Tariff (\url{https://cran.r-project.org/package=Tariff}) and the
   ...
   several additional steps to work with ODK data exports of the WHO VA instruments and thus are not included in the app.% i am not sure if this shoudl be part of the list (i.e. (C)) i think it should be more of a Technical note:)
  \end{enumerate}
...
ODKBriefcase export and included in the \R Shiny zip file (the corresponding input option is selected by default). %The () is confusing and I woudl remove.
...
\item {\bf Download Causes for All Records as .csv} -- This file includes the record ID (which is simply the row number %this row number is an isuse and in the long run, i think we should allow for the following: after the user selects the data set, they should be given a drop down menue with the headres of all columns and they should be asked to identify the colomun which is the unique ID for each of the VAs, i.e. the linking identifier (i am not sure how easy that is to implement but it would make the individula level output much more useful)
  ...
  among indicators that cannot be reconciled). %the "cannot be reconciled" may be confusing. should these not come out as undetermined
...
\item The three options listed above are available for each group included in the analysis. % it is not clear to me what this is referring to
